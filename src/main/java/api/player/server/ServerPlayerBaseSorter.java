...
public final class ServerPlayerBaseSorter {
    ...
    public void Sort() {
        ...
        if (explicitInferiors != null) explicitInferiors.clear();

        if (explicitSuperiors != null) explicitSuperiors.clear();

        if (directInferiorsMap != null) directInferiorsMap.clear();

        if (allInferiors != null) allInferiors.clear();
                ...
                for (int n = i + 1; n < list.size(); n++) {
                    ...
                    if (leftWantsToBeInferiorToRight && leftWantsToBeSuperiorToRight)
                        throw new UnsupportedOperationException(
                                "Can not sort ServerPlayerBase classes for method '" + methodName + "'. '" + left
                                        + "' wants to be superior and inferior to '" + right + "'");
                    if (rightWantsToBeInferiorToLeft && rightWantsToBeSuperiorToLeft)
                        throw new UnsupportedOperationException(
                                "Can not sort ServerPlayerBase classes for method '" + methodName + "'. '" + right
                                        + "' wants to be superior and inferior to '" + left + "'");
                }

            if (allInferiors == null) allInferiors = new Hashtable<String, Set<String>>();

            for (int i = 0; i < list.size(); i++) build(list.get(i), null);
        }

        if (withoutSuperiors == null) withoutSuperiors = new LinkedList<String>();
            ...
            for (int i = offset; i < offset + size; i++) {
                ...
                if (withoutSuperiors.contains(key)) {
                    if (initial) {
                        ...
                        if (allInferiors != null) inferiors = allInferiors.get(key);
    ...
    private Set<String> build(String type, String startType) {
        ...
        if (inferiors == null) {
            ...
            if (inferiors == null) inferiors = Empty;
    ...
    private Set<String> build(String type, Set<String> inferiors, String startType) {
        ...
        if (directInferiors == null) return inferiors;

        if (inferiors == null) inferiors = new HashSet<String>();
        ...
        while (iter.hasNext()) {
            ...
            try {
                ...
                throw new UnsupportedOperationException(
                        "Can not sort ServerPlayerBase classes for method '" + methodName
                                + "'. Circular superiosity found including '" + inferiorType + "'",
                        uoe);
            }

            if (inferiorSet != Empty) inferiors.addAll(inferiorSet);
        }
        return inferiors;
    }
    ...
    private static Map<String, Set<String>> build(
        ...
        for (int n = 0; n < names.length; n++) {
            if (names[n] != null) types.add(names[n]);
        }

        if (directMap != null) getOrCreateSet(directMap, baseId).addAll(types);

        if (otherDirectMap != null) {
            Iterator<String> iter = types.iterator();
            while (iter.hasNext()) getOrCreateSet(otherDirectMap, iter.next()).add(baseId);
        }
